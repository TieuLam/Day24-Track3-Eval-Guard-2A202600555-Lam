...
# ─── Helper: pointwise judge (cho Cohen's κ vs human labels) ──────────────────

def single_answer_judge(question: str, answer: str, ground_truth: str) -> dict:
    """Chấm 1 câu trả lời là TỐT (1) hay XẤU (0) bằng cách đối chiếu ground_truth.

    Dùng để sinh judge_labels so khớp với human_label → tính Cohen's κ.
    So với cách cũ (so model_answer với 1 câu giả luôn thua → mọi nhãn = 1, κ=0),
    cách này có tham chiếu thực nên judge phân biệt được đúng/sai → κ có ý nghĩa.

    Returns:
        {"label": 0|1, "reasoning": str}
    """
    PROMPT = '''Bạn là expert đánh giá câu trả lời của trợ lý chính sách nhân sự.
...
Câu trả lời có CHÍNH XÁC và ĐẦY ĐỦ so với đáp án chuẩn không?
(Sai số liệu, sai người phê duyệt, sai kết luận → label 0. Đúng về bản chất → label 1.)
Trả lời JSON (chỉ JSON): {{"label": 0 hoặc 1, "reasoning": "giải thích ngắn"}}'''
    ...
    resp = client.chat.completions.create(
        model=JUDGE_MODEL,
        messages=[
            {"role": "system", "content": "Bạn là expert đánh giá HR. Chỉ trả lời JSON."},
...
if __name__ == "__main__":
    ...
    # Map question_id → ground_truth từ test set (để judge có tham chiếu thực)
    with open(TEST_SET_PATH, encoding="utf-8") as f:
        gt_map = {x["id"]: x["ground_truth"] for x in json.load(f)}

    # judge_labels: pointwise judge model_answer vs ground_truth (1 = đúng, 0 = sai)
    # judge_results: pairwise swap-and-average (model_answer vs ground_truth) cho bias_report
    judge_labels = []
    judge_results = []
    for item in human_data:
        q  = item["question"]
        ma = item["model_answer"]
        gt = gt_map.get(item["question_id"], "")

        verdict = single_answer_judge(q, ma, gt)
        judge_labels.append(verdict["label"])

        # Pairwise model_answer (A) vs ground_truth (B) → tín hiệu position/verbosity bias thực
        res = swap_and_average(q, ma, gt)
        judge_results.append(res)

    kappa = cohen_kappa(judge_labels, human_labels)
    agree = sum(1 for j, h in zip(judge_labels, human_labels) if j == h)
    print(f"Judge vs human: {agree}/{len(human_labels)} khớp | Cohen's κ: {kappa:.3f}")
    ...
    report = {
        ...
        "agreement": f"{agree}/{len(human_labels)}",
        "bias_report": bias,
        "judge_results": [
            {
                "question": r.question,
