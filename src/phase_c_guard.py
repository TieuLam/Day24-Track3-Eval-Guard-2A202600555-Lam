...
# Chỉ tính các loại PII THỰC SỰ nhạy cảm. Model spaCy tiếng Anh hay mis-tag từ
# tiếng Việt thường (vd "nghỉ phép") là PERSON và bắt năm "2024" là DATE_TIME →
# gây false positive. Lọc theo allowlist + ngưỡng score để loại nhiễu.
RELEVANT_PII_ENTITIES = {
    "VN_CCCD", "VN_PHONE", "EMAIL_ADDRESS", "CREDIT_CARD",
    "IBAN_CODE", "IP_ADDRESS", "CRYPTO", "US_SSN",
}
...
def pii_scan(text: str, analyzer=None, anonymizer=None) -> dict:
    ...
    # Giữ lại PII có ý nghĩa: đúng loại nhạy cảm + đủ tin cậy
    results = [r for r in raw
               if r.entity_type in RELEVANT_PII_ENTITIES and r.score >= PII_SCORE_THRESHOLD]
    if not results:
...
def _extract_text(response) -> str:
    """Lấy text từ GenerationResponse / dict / list message của NeMo 0.22."""
    resp = getattr(response, "response", response)
...
def _rail_stopped(response, rail_type: str) -> bool:
    """True nếu NeMo kích hoạt một rail đúng loại và DỪNG (chặn) request.

    Cách tin cậy ở NeMo 0.22: canonical-form matching bằng embedding tiếng Anh không
    phân biệt được tiếng Việt và không đưa text refuse vào response → không thể dò bằng
    keyword. Dùng options log để đọc activated_rails[*].stop.
    """
    log = getattr(response, "log", None)
    activated = getattr(log, "activated_rails", None) or []
    return any(getattr(r, "type", None) == rail_type and getattr(r, "stop", False)
               for r in activated)


async def check_input_rail(text: str, rails=None) -> dict:
    """Task 9b: Kiểm tra input qua NeMo input rails (self check input — LLM-based).

    Returns:
        {
          "allowed":        bool,
          "blocked_reason": str | None,
          "response":       str,          # NeMo's raw response text
        }
    """
    if rails is None:
    ...
    # Ưu tiên tín hiệu rail.stop (tin cậy); fallback dò refuse keyword nếu không có log
    blocked = _rail_stopped(response, "input") or \
        any(kw in response_text.lower() for kw in REFUSE_KEYWORDS)
    return {
        "allowed":        not blocked,
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    response_text = _extract_text(response)
    # output rail self_check_output dừng → flagged; fallback dò refuse keyword
    flagged = _rail_stopped(response, "output") or \
        any(kw in response_text.lower() for kw in REFUSE_KEYWORDS)
    return {
        "safe":           not flagged,
...
def measure_p95_latency(test_inputs: list[str], n_runs: int = 20,
    ...
    async def _measure():
        # Warm-up: lần analyze()/generate đầu chịu chi phí lazy-load của spaCy/NeMo.
        # Bỏ qua nó để P95 phản ánh steady-state production, không phải cold-start.
        if test_inputs:
            pii_scan(test_inputs[0], analyzer, anonymizer)
            await check_input_rail(test_inputs[0], rails)
...
if __name__ == "__main__":
    # Khởi tạo singleton một lần để tái sử dụng — tránh re-init engine mỗi call
    # (đây là điều kiện để đo latency đúng: Presidio chỉ nhanh khi không build lại engine)
    analyzer, anonymizer = setup_presidio()
    rails = setup_nemo_rails()
    ...
    # Task 12: P95 latency — truyền singleton để Presidio không khởi tạo lại mỗi vòng
